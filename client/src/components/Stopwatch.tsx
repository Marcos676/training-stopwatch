...
export default function Stopwatch(): any {
  ...
  function formatTime(time: number): Array<string> {
    ...
    let result = [hours, minutes, seconds, milliseconds].map((unit) => String(unit).padStart(2, "0"));
  ...
  return (
    <section className="stopwatch w-full flex flex-col items-center justify-center max-w-90">
      <div className="stopwatch_display text-white font-mono flex gap-1">
        <span className="stopwatch_hours text-2xl ">
          {hours}
        </span>
        <span className="stopwatch_minutes text-7xl min-[360px]:text-8xl">
          {`${minutes}:${seconds}`}
        </span>
        <span className="stopwatch_milliseconds text-2xl ">
          {milliseconds}
        </span>
      </div>
      <div className="stopwatch_controls w-full flex justify-between mt-4">
        <Button bgAndHoverColors="bg-green-500 hover:bg-green-700" text="START" click={handleStart} />
        <Button bgAndHoverColors="bg-red-500 hover:bg-red-700" text="STOP" click={handleStop} />
        <Button bgAndHoverColors="bg-gray-500 hover:bg-gray-700" text="RESET" click={handleReset} />
      </div>
    </section>
  );
}
